...
void sfzero::Reader::read(const juce::File &file) {
  ...
  read(static_cast<const char *>(contents.getData()),
      ...
      // Parameter.
        ...
        if (inControl) {
          if (opcode == "default_path") {
            ...
            juce::String fauxOpcode =
                juce::String(opcode.getStart(), opcode.length()) +
                " (in <control>)";
          ...
          if (buildingRegion == nullptr) {
            ...
            buildingRegion->hikey = buildingRegion->lokey =
            ...
            buildingRegion->trigger =
                static_cast<sfzero::Region::Trigger>(triggerValue(value));
            ...
            bool modeIsSupported = value == "no_loop" || value == "one_shot" ||
                                   value == "loop_continuous";
            if (modeIsSupported) {
              buildingRegion->loop_mode =
                  static_cast<sfzero::Region::LoopMode>(loopModeValue(value));
            } else {
              juce::String fauxOpcode =
                  juce::String(opcode.getStart(), opcode.length()) + "=" +
                  value;
...
const char *sfzero::Reader::readPathInto(juce::String *pathOut, const char *pIn,
                                         const char *endIn) {
