...
void sfzero::Voice::startNote(int midiNoteNumber, float floatVelocity,
                              juce::SynthesiserSound *soundIn,
  ...
  if ((region_ == nullptr) || (region_->sample == nullptr) ||
      (region_->sample->getBuffer() == nullptr)) {
    killNote();
    return;
  }
  ...
  double velocityGainDB =
      -20.0 * log10((127.0 * 127.0) / (velocity * velocity));
  velocityGainDB *= region_->amp_veltrack / 100.0;
  noteGainDB += velocityGainDB;
  noteGainLeft_ = noteGainRight_ =
      static_cast<float>(juce::Decibels::decibelsToGain(noteGainDB));
  ...
  ampeg_.startNote(&region_->ampeg, floatVelocity, getSampleRate(),
                   &region_->ampeg_veltrack);
  ...
  if ((loopMode != sfzero::Region::no_loop) &&
      (loopMode != sfzero::Region::one_shot)) {
...
void sfzero::Voice::renderNextBlock(juce::AudioSampleBuffer &outputBuffer,
  ...
  const float *inR =
      buffer->getNumChannels() > 1 ? buffer->getReadPointer(1, 0) : nullptr;
  ...
  while (--numSamples >= 0) {
    ...
    float nextR =
        inR ? (nextPos < bufferNumSamples ? inR[nextPos] : inR[pos]) : nextL;
    float l = (inL[pos] * invAlpha + nextL * alpha);
    float r = inR ? (inR[pos] * invAlpha + nextR * alpha) : l;

    //// Simple linear interpolation, old version (possible buffer overrun with
    /// non-loop??)
...
bool sfzero::Voice::isPlayingNoteDown() {
  return region_ && region_->trigger != sfzero::Region::release;
}

bool sfzero::Voice::isPlayingOneShot() {
  return region_ && region_->loop_mode == sfzero::Region::one_shot;
}
...
void sfzero::Voice::setRegion(sfzero::Region *nextRegion) {
  region_ = nextRegion;
}

juce::String sfzero::Voice::infoString() {
  const char *egSegmentNames[] = {"delay",   "attack",  "hold", "decay",
                                  "sustain", "release", "done"};

  const static int numEGSegments(sizeof(egSegmentNames) /
  ...
  juce::String info;
  info << "note: " << curMidiNote_ << ", vel: " << curVelocity_
       << ", pan: " << region_->pan << ", eg: " << egSegmentName
       << ", loops: " << numLoops_;
  return info;
}
...
void sfzero::Voice::calcPitchRatio() {
  ...
  double adjustedPitch =
      ...
      (note - region_->pitch_keycenter) * (region_->pitch_keytrack / 100.0);
  ...
  double naturalFreq =
      juce::MidiMessage::getMidiNoteInHertz(region_->pitch_keycenter);
  pitchRatio_ = (targetFreq * region_->sample->getSampleRate()) /
