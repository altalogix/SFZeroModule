...
sfzero::SF2Sound::SF2Sound(const juce::File &file)
    : sfzero::Sound(file), selectedPreset_(0) {}
...
sfzero::SF2Sound::~SF2Sound() {
  ...
  for (juce::HashMap<int, sfzero::Sample *>::Iterator i(samplesByRate_);
       i.next();) {
...
class PresetComparator {
 public:
  static int compareElements(const sfzero::SF2Sound::Preset *first,
                             const sfzero::SF2Sound::Preset *second) {
...
void sfzero::SF2Sound::loadSamples(juce::AudioFormatManager * /*formatManager*/,
...
void sfzero::SF2Sound::addPreset(sfzero::SF2Sound::Preset *preset) {
  presets_.add(preset);
}
