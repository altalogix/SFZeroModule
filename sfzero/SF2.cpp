...
void sfzero::SF2::Hydra::readFrom(juce::InputStream *file,
                                  juce::int64 pdtaChunkEnd) {
  int i, numItems;

#define HandleChunk(chunkName)                                      \
  if (FourCCEquals(chunk.id, #chunkName)) {                         \
    numItems = std::trunc(chunk.size / SF2::chunkName::sizeInFile); \
  ...
  while (file->getPosition() < pdtaChunkEnd) {
    ...
    HandleChunk(phdr) HandleChunk(pbag) HandleChunk(pmod) HandleChunk(pgen)
        HandleChunk(inst) HandleChunk(ibag) HandleChunk(imod) HandleChunk(igen)
            HandleChunk(shdr) {}
...
bool sfzero::SF2::Hydra::isComplete() {
  return phdrItems && pbagItems && pmodItems && pgenItems && instItems &&
         ibagItems && imodItems && igenItems && shdrItems;
}
