...
sfzero::EG::EG()
    : segment_(),
      sampleRate_(0),
      exponentialDecay_(false),
      level_(0),
      slope_(0),
      samplesUntilNextSegment_(0),
      segmentIsExponential_(false) {}

void sfzero::EG::setExponentialDecay(bool newExponentialDecay) {
  exponentialDecay_ = newExponentialDecay;
}

void sfzero::EG::startNote(const EGParameters *newParameters,
                           float floatVelocity, double newSampleRate,
...
void sfzero::EG::startDelay() {
  if (parameters_.delay <= 0) {
    ...
    samplesUntilNextSegment_ =
        static_cast<int>(parameters_.delay * sampleRate_);
    segmentIsExponential_ = false;
  }
}
...
void sfzero::EG::startAttack() {
  if (parameters_.attack <= 0) {
    ...
    samplesUntilNextSegment_ =
        static_cast<int>(parameters_.attack * sampleRate_);
...
void sfzero::EG::startDecay() {
  if (parameters_.decay <= 0) {
    ...
    segment_ = Decay;
    samplesUntilNextSegment_ =
        static_cast<int>(parameters_.decay * sampleRate_);
    level_ = 1.0;
    if (exponentialDecay_) {
      ...
      if (parameters_.sustain > 0.0) {
        ...
        samplesUntilNextSegment_ = static_cast<int>(
            log((parameters_.sustain / 100.0) / level_) / mysterySlope);
