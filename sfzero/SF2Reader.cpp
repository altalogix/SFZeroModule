...
sfzero::SF2Reader::SF2Reader(sfzero::SF2Sound *soundIn,
                             const juce::File &fileIn)
    : sound_(soundIn) {
  file_ = fileIn.createInputStream().get();
}
...
void sfzero::SF2Reader::read() {
  ...
  // Read each preset.
  for (int whichPreset = 0; whichPreset < hydra.phdrNumItems - 1;
       ++whichPreset) {
    sfzero::SF2::phdr *phdr = &hydra.phdrItems[whichPreset];
    sfzero::SF2Sound::Preset *preset = new sfzero::SF2Sound::Preset(
        phdr->presetName, phdr->bank, phdr->preset);
      ...
      for (int whichGen = pbag->genNdx; whichGen < genEnd; ++whichGen) {
        ...
        if (pgen->genOper == sfzero::SF2Generator::instrument) {
          ...
          if (whichInst < hydra.instNumItems) {
            ...
            for (int whichZone2 = firstZone; whichZone2 < zoneEnd2;
                 ++whichZone2) {
              ...
              for (int whichGen2 = ibag->instGenNdx; whichGen2 < genEnd2;
                   ++whichGen2) {
                ...
                if (igen->genOper == sfzero::SF2Generator::sampleID) {
                  ...
                  *newRegion = zoneRegion;
                  ...
                  addGeneratorToRegion(igen->genOper, &igen->genAmount,
                                       &zoneRegion);
                }
              }
...
juce::AudioSampleBuffer *sfzero::SF2Reader::readSamples(double *progressVar,
                                                        juce::Thread *thread) {
  ...
  int numSamples = std::trunc(chunk.size / sizeof(short));
  juce::AudioSampleBuffer *sampleBuffer =
      new juce::AudioSampleBuffer(1, numSamples);
...
void sfzero::SF2Reader::addGeneratorToRegion(sfzero::word genOper,
                                             sfzero::SF2::genAmountType *amount,
  ...
  switch (genOper) {
    ...
    case sfzero::SF2Generator::sampleModes: {
      sfzero::Region::LoopMode loopModes[] = {
          sfzero::Region::no_loop, sfzero::Region::loop_continuous,
          sfzero::Region::no_loop, sfzero::Region::loop_sustain};
    ...
    case sfzero::SF2Generator::unused5: {
      const sfzero::SF2Generator *generator =
          sfzero::GeneratorFor(static_cast<int>(genOper));
