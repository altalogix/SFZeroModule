...
sfzero::Sound::~Sound() {
  ...
  for (juce::HashMap<juce::String, sfzero::Sample *>::Iterator i(samples_);
       i.next();) {
    delete i.getValue();
  }
}
...
sfzero::Sample *sfzero::Sound::addSample(juce::String path,
...
void sfzero::Sound::addError(const juce::String &message) {
  errors_.add(message);
}
...
void sfzero::Sound::loadSamples(juce::AudioFormatManager *formatManager,
                                double *progressVar, juce::Thread *thread) {
  if (progressVar) {
    *progressVar = 0.0;
  }
...
sfzero::Region *sfzero::Sound::getRegionFor(int note, int velocity,
...
juce::String sfzero::Sound::subsoundName(int /*whichSubsound*/) {
  return juce::String();
}
